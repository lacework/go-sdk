...
filter on containers with vulnerabilities that have fixes available.`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			//todo(v2): json output
			//if cli.JSONOutput() {
			//	var assesmentsJsonOutput []api.VulnerabilityContainer
			//	for _, a := range assessments {
			//		assesmentsJsonOutput = append(assesmentsJsonOutput, a)
			//	}
			//	return cli.OutputJSON(assesmentsJsonOutput)
			//}
...
func buildVulnCtrAssessmentSummary(assessments []api.VulnerabilityContainer) (uniqueAssessments []vulnerabilityAssessmentSummary) {
	...
	for _, a := range assessments {
		...
		imageMap[i] = vulnerabilityAssessmentSummary{a.ImageID,
...
type vulnCtrAssessment struct {
	id             string
	ctrs           []api.VulnerabilityContainer
	uniqueCtrCount int
	image          api.ImageInfo
	startTime      time.Time
	status         string
