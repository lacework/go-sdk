...
type testParams struct {
	cloud    string   // required: the cloud provider under test
	location string   // where to put the terraform code
	flags    []string // flags to pass to the Lacework CLI
}

type testOptions func(*testParams)

func withCloud(cloud string) func(params *testParams) {
	return func(params *testParams) {
		params.cloud = cloud
	}
}

func withLocation(location string) func(params *testParams) {
	return func(params *testParams) {
		params.location = location
	}
}

func withFlags(flags ...string) func(params *testParams) {
	return func(params *testParams) {
		params.flags = flags
	}
}

// run a test for lacework generate
func runGenerateTest(t *testing.T, conditions func(*expect.Console), options ...testOptions) string {
	params := testParams{}
	for _, opt := range options {
		opt(&params)
	}
	// Validate args
	validClouds := []string{"aws", "azure", "gcp", "oci"}
	assert.Contains(t, validClouds, params.cloud, "cloud must be one of %v", validClouds)
	if params.location == "" {
		params.location = filepath.Join(tfPath, fmt.Sprintf("lacework/%s", params.cloud))
	} else {
		res, err := os.Stat(params.location)
		assert.Nil(t, err, "invalid output location")
		assert.Equal(t, true, res.IsDir(), "output location must be a directory")
	}

	os.Setenv("HOME", tfPath)
	cmdArgs := append([]string{"generate", "cloud-account", params.cloud}, params.flags...)
	runFakeTerminalTestFromDir(t, tfPath, conditions, cmdArgs...)
	out, err := os.ReadFile(filepath.Join(params.location, "main.tf"))
	if err != nil {
		return fmt.Sprintf("main.tf not found: %s", err)
	}

	t.Cleanup(func() {
		os.Remove(filepath.Join(params.location, "main.tf"))
	})

	result := terraformValidate(params.location)
